...
""" NOTES """
# 1. XXXX represents failed translations because the values don't exist in the postgres look_up table.
...
def translator(x, value):
	...
	if x == 3:
		...
		cur1.execute("SELECT item_id, item_description, item_description_short, item_category, item_sub_category, field_name FROM list_general " +
					 "WHERE LOWER(item_description) LIKE '" + value.lower() + "%' AND item_category='Monitor' AND field_name LIKE 'olmis%'")
		if cur1.rowcount > 0:
...
try:
	...
	# conn1 = pymssql.connect(host='localhost', user='DESKTOP-N2TORCL\Palladium', password='fg', database='APHIAMAINDB')
	conn1 = pymssql.connect(host=r'localhost', user=r'olmis_user',password=r'P@55w0rd', database=r'APHIAMAINDB')
	#conn1 = pymssql.connect(host='192.168.42.243:1433', user='olmis_user', password='P@55w0rd', database='APHIAMAINDB')
	...
	""" flatten the dataset """
	cur1.execute("SELECT TOP 5  Clientdetails.ClientID AS ClientID, Clientdetails.OVCID AS OVCID, Clientdetails.FirstName AS ClientFirstName," +
				 ...
				 "ParentDetails.IsDead, ParentDetails.CauseofDeath,ParentDetails.HIVStatus AS ParentHIVStatus, ParentDetails.Gender AS ParentGender," +
				 ...
				 "ParentDetails.Schoollevel AS ParentSchoolLevel, ParentDetails.ChildreninHousehold AS ParentChildreninHousehold, ParentDetails.ChildreninOVCProg AS ParentChildrenOVCProg," +
				 ...
				 "District.District AS ParentDistrict, CBO.CBO AS CBOName " +
				 ...
				 # "WHERE OVCID IN('20060215-20161130101229')")
	...
	for row in cur1:
		...
		cur2.execute("INSERT INTO reg_person(first_name, other_names, surname, designation, sex_id, date_of_birth, is_void, created_at) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;",
					 (cfirst_name, cother_names, csurname, cdesignation, csex_id, cdate_of_birth, is_void, created_at))
		##conn2.commit()
		person_pk = cur2.fetchone()[0]
		print 'inserted into reg_person(ovc) . . .'
		...
		date_ended = None
		person_id = person_pk
		cur2.execute("INSERT INTO reg_persons_types(person_type_id, date_began, date_ended, person_id, is_void) VALUES(%s, %s, %s, %s, %s) RETURNING id;",
					 (person_type_id, date_began, date_ended, person_id, is_void))
		##conn2.commit()
		print 'inserted into reg_persons_types(ovc) . . .'

		# insert into reg_persons_geo_client
		date_linked = created_at
		date_delinked = None
		area_type = 'GPRV'
		area_id = row['ClientDistrict']
		person_id = person_pk
		cur2.execute("INSERT INTO reg_persons_geo(date_linked, date_delinked, area_type, area_id, person_id, is_void) VALUES(%s, %s, %s, %s, %s, %s) RETURNING id;",
					 (date_linked, date_delinked, area_type, area_id, person_id, is_void))
		##conn2.commit()
		print 'inserted into reg_persons_geo(ovc) . . .'
		...
		if not (row['ClientHouseHoldheadID']) in HouseHoldheadIDs:
			...
			##conn2.commit()
			parent_pk = cur2.fetchone()[0]
			print 'inserted into reg_person(parent) . . .'

			# insert into reg_person_parent
			identifier_type_id='INTL'
			identifier=row['ParentIDNumber']
			person_id = parent_pk
			cur2.execute("INSERT INTO reg_persons_external_ids(identifier_type_id, identifier, is_void, person_id) VALUES(%s, %s, %s, %s) RETURNING id;",
						 (identifier_type_id, identifier, is_void, person_id))
			##conn2.commit()
			print 'inserted into reg_persons_external_ids . . .'
			...
			cur2.execute("INSERT INTO ovc_household(id, head_identifier, created_at, is_void, head_person_id) VALUES(%s, %s, %s, %s, %s) RETURNING id;",
						(hash_key, head_identifier, created_at, is_void, head_person_id))
			##conn2.commit()
			household_pk = cur2.fetchone()[0]
			...
			print 'inserted into ovc_household . . .'
			...
			person_id=parent_pk
			cur2.execute("INSERT INTO ovc_household_members(id, hh_head, member_type, date_linked, date_delinked, is_void,house_hold_id, person_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;",
						(hash_key, hh_head, member_type, date_linked, date_delinked, is_void,house_hold_id, person_id))
			##conn2.commit()
			print 'inserted into ovc_household_members (parent) . . .'
			...
			cur2.execute("INSERT INTO reg_persons_types(person_type_id, date_began, date_ended, person_id, is_void) VALUES(%s, %s, %s, %s, %s) RETURNING id;",
						 (person_type_id, date_began, date_ended, person_id, is_void))
			##conn2.commit()
			print 'inserted into reg_persons_types . . .'
			...
			cur2.execute("INSERT INTO reg_persons_geo(date_linked, date_delinked, area_type, area_id, person_id, is_void) VALUES(%s, %s, %s, %s, %s, %s) RETURNING id;",
						 (date_linked, date_delinked, area_type, area_id, person_id, is_void))
			#conn2.commit()
			print 'inserted into reg_persons_geo(parent) . . .'
			...
			cur2.execute("INSERT INTO reg_persons_guardians(date_linked, date_delinked, relationship, child_headed, child_person_id, guardian_person_id, is_void) VALUES(%s, %s, %s, %s, %s, %s, %s) RETURNING id;",
						 (date_linked, date_delinked, relationship, child_headed, child_person_id, guardian_person_id, is_void))
			##conn2.commit()
			print 'inserted into reg_persons_guardians . . .'
			...
			for d in dictXarray:
				if d['householdhead_id'] == row['ClientHouseHoldheadID']:
					...
					if child_person_id != person_pk:
						cur2.execute("INSERT INTO reg_persons_siblings(date_linked, date_delinked, remarks, child_person_id, sibling_person_id, is_void) VALUES(%s, %s, %s, %s, %s, %s) RETURNING id;",
									 (date_linked, date_delinked, remarks, d['child_person_id'], sibling_person_id, is_void))
						##conn2.commit()
						print 'inserted into reg_persons_siblings . . .'
					hash_key = uuid.uuid4()
					psycopg2.extras.register_uuid()
					hh_head=False
					member_type='TBVC'
					...
					cur2.execute("INSERT INTO ovc_household_members(id, hh_head, member_type, date_linked, date_delinked, is_void,house_hold_id, person_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;",
								 (hash_key, hh_head, member_type, date_linked, date_delinked, is_void,house_hold_id, person_id))
					##conn2.commit()
					print 'inserted into ovc_household_members(ovc). . .'
		
		# insert into reg_org_unit
		org_unit_id = None
		org_unit_name = None
		cur2.execute("SELECT id, org_unit_id_vis, org_unit_name FROM reg_org_unit WHERE org_unit_name='" + row['CBOName'] + "'")
		if cur2.rowcount > 0:
			for r in cur2:			
				org_unit_id = r[0]
				org_unit_name = r[2]
			...
			created_by_id = 1
			org_unit_name = row['CBOName']
			cur2.execute("INSERT INTO reg_org_unit(org_unit_id_vis, org_unit_name, org_unit_type_id, date_operational, date_closed, handle_ovc, is_void, parent_org_unit_id, created_at, created_by_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;",
			...
			##conn2.commit()
			org_unit_pk = cur2.fetchone()[0]
			org_unit_name =  row['CBOName']
			# CBONames.append(org_unit_name)
			print 'inserted into reg_org_unit . . .'

			# insert into reg_org_units_geo
			date_linked = created_at
			date_delinked = None
			area_id = row['ParentCbo']
			org_unit_id = org_unit_pk
			cur2.execute("INSERT INTO reg_org_units_geo(date_linked, date_delinked, area_id, org_unit_id, is_void) VALUES(%s, %s, %s, %s, %s) RETURNING id;",
						 (date_linked, date_delinked, area_id, org_unit_id, is_void))
			#conn2.commit()
			print 'inserted into reg_org_units_geo . . .'
		...
		hash_key = uuid.uuid4()
		psycopg2.extras.register_uuid()
		cur2.execute("INSERT INTO ovc_registration(id, registration_date, has_bcert, is_disabled, hiv_status, school_level, immunization_status, org_unique_id, exit_date, created_at, is_void, caretaker_id, child_cbo_id, child_chv_id, person_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
		...
		#conn2.commit()
		print 'inserted into ovc_registration . . .'
	...
	#-------------------------------------------------------------------------
	""" CSI Data """
	cur1_csi = conn1.cursor(as_dict=True)
	cur1_csi2 = conn1.cursor(as_dict=True)
	cur1_csi3 = conn1.cursor(as_dict=True)
	for data in OVCIDs:
		person_pk = data['person_pk']
		OVCID = data['OVCID']
		cur1_csi.execute("SELECT NeedsAssessmentMain.NeedsAssessmentID AS NeedsAssessmentID, NeedsAssessmentMain.DateofAssessment AS DateofAssessment, NeedsAssessmentMain.Is_InitialAssessment AS Is_InitialAssessment FROM NeedsAssessmentMain WHERE NeedsAssessmentMain.OVCID='" + OVCID + "'")
		for row in cur1_csi.fetchall():
			if row['NeedsAssessmentID'] not in NeedsAssessmentPKs:
				NeedsAssessmentPKs.append(row['NeedsAssessmentID'])
	...
	for OVCCareEvents in OVCCareEvents_CSI:
		...
		house_hold = None
		cur2.execute("INSERT INTO ovc_care_events(event, event_type_id, event_counter, event_score, date_of_event, is_void, person_id, house_hold_id, timestamp_created, created_by, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING event;",
		...
		#conn2.commit()
		event_pk = cur2.fetchone()[0]
		print 'inserted into ovc_care_events(CSI) . . .'

		# TAB1
		cur1_csi.execute("SELECT NeedsAssessmentMain.NeedsAssessmentID AS NeedsAssessmentID, NeedsAssessmentMain.OVCID AS OVCID, NeedsAssessmentMain.DateofAssessment AS DateofAssessment, " +
		...
		for row in cur1_csi:
			...
			is_void = False
			sync_id = hash_key2
			cur2.execute("INSERT INTO ovc_care_eav(eav_id, entity, attribute, value, value_for, event_id, is_void, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) RETURNING eav_id;",
						 (eav_id, entity, attribute, value, value_for, event, is_void, sync_id))
			#conn2.commit()
			print 'inserted into ovc_care_eav(CSI) . . .'

		# TAB2
		cur1_csi2.execute("SELECT NeedsAssessmentMain.NeedsAssessmentID, NeedsAssessmentMain.OVCID, NeedsAssessmentMain.DateofAssessment," +
		...
		for row in cur1_csi2:
			...
			is_void = False
			sync_id = hash_key2
			cur2.execute("INSERT INTO ovc_care_priority(priority_id, domain, service, service_grouping_id, event_id, is_void, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s) RETURNING priority_id;",
						 (priority_id, domain, service, service_grouping_id, event, is_void, sync_id))
			#conn2.commit()
			print 'inserted into ovc_care_priority(CSI) . . .'

		# TAB3
		cur1_csi3.execute("SELECT NeedsAssessmentMain.NeedsAssessmentID, NeedsAssessmentMain.OVCID, NeedsAssessmentMain.DateofAssessment," +
		...
		for row in cur1_csi3:
			...
			is_void = False
			sync_id = hash_key3
			cur2.execute("INSERT INTO ovc_care_services(service_id, service_provided, service_provider, place_of_service, date_of_encounter_event, service_grouping_id, event_id, is_void, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING service_id;",
			...
			#conn2.commit()
			print 'inserted into ovc_care_services(CSI) . . .'

	""" F1A Data """
	cur1_f1a = conn1.cursor(as_dict=True)
	cur1_f1a2 = conn1.cursor(as_dict=True)
	for data in OVCIDs:
		person_pk = data['person_pk']
		OVCID = data['OVCID']
		cur1_f1a.execute(
			"SELECT SSVID, DateofVisit, VisitType FROM StatusAndServiceVisit WHERE OVCID='" + OVCID + "'")
		for row in cur1_f1a.fetchall():
			if row['SSVID'] not in ServicePKs:
				ServicePKs.append(row['SSVID'])
	...
	for OVCCareEvents in OVCCareEvents_F1A:
		...
		#conn2.commit()
		event_pk = cur2.fetchone()[0]
		print 'inserted into ovc_care_events(F1A) . . .'

		# TAB 1
		cur1_f1a2.execute("SELECT StatusAndServiceVisit.SSVID, StatusAndServiceVisit.OVCID, StatusAndServiceVisit.DateofVisit, StatusAndServiceVisit.VisitType, StatusAndServiceMonitoring_Assessment.SSID," +
		...
		for row in cur1_f1a2:
			...
			is_void = True
			sync_id = hash_key3
			cur2.execute("INSERT INTO ovc_care_assessment(assessment_id, domain, service, service_status, service_grouping_id, is_void, sync_id, event_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) RETURNING assessment_id;",
					 (assessment_id, domain, service, service_status, service_grouping_id, is_void, sync_id, event))
			#conn2.commit()
			print 'inserted into ovc_care_assessment(F1A) . . .'

		#TAB2
		cur1_f1a2.execute("SELECT StatusAndServiceVisit.SSVID, StatusAndServiceVisit.OVCID, StatusAndServiceVisit.DateofVisit," +
		...
		for row in cur1_f1a2:
			...
			cur2.execute("INSERT INTO ovc_care_eav(eav_id, entity, attribute, value, value_for, event_id, is_void, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) RETURNING eav_id;",
						 (eav_id, entity, attribute, value, value_for, event, is_void, sync_id))
			#conn2.commit()
			print 'inserted into ovc_care_eav(F1A) . . .'
		...
		for row in cur1_f1a2:
			...
			cur2.execute("INSERT INTO ovc_care_priority(priority_id, domain, service, service_grouping_id, event_id, is_void, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s) RETURNING priority_id;",
						 (priority_id, domain, service, service_grouping_id, event, is_void, sync_id))
			#conn2.commit()
			print 'inserted into ovc_care_priority(F1A) . . .'

		#TAB4
		cur1_f1a2.execute("SELECT StatusAndServiceVisit.SSVID, StatusAndServiceVisit.OVCID, StatusAndServiceVisit.DateofVisit, StatusAndServiceVisit.VisitType, StatusAndServiceMonitoring_Service.SSID," +
						  "ServiceStatus.ServiceStatus AS ServiceStatus, ServiceStatus.CSID, CoreServices.CoreService AS CoreService, Domain.Domain FROM StatusAndServiceVisit INNER JOIN " +
		...
		for row in cur1_f1a2:
			...
			cur2.execute("INSERT INTO ovc_care_services(service_id, service_provided, service_provider, place_of_service, date_of_encounter_event, service_grouping_id, event_id, is_void, sync_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING service_id;",
						 (service_id, service_provided, service_provider, place_of_service, date_of_encounter_event, service_grouping_id, event, is_void, sync_id))
			#conn2.commit()
			print 'inserted into ovc_care_services(F1A) . . .'

	""" HHVA Data """
	cur1_hhva = conn1.cursor(as_dict=True)
	cur2_hhva = conn1.cursor(as_dict=True)
	for data in OVCIDs:
		person_pk = data['person_pk']
		household_pk = data['household_pk']
		OVCID = data['OVCID']
		cur1_hhva.execute("SELECT HHAssessmentid, OVCID, DateofAssessment, Is_InitialAssessment FROM HHAssessmentMain WHERE OVCID='" + OVCID + "'")
		for row in cur1_hhva.fetchall():
	...
	for OVCCareEvents in OVCCareEvents_HHVA:
		...
		#conn2.commit()
		event_pk = cur2.fetchone()[0]
		print 'inserted into ovc_care_events(HHVA) . . .'

		
		cur2_hhva.execute("SELECT HHAssessmentMain.HHAssessmentid, HHAssessmentMain.OVCID, HHAssessmentMain.DateofAssessment," +
